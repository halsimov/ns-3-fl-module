...
TypeId
PerformanceSimpleModel::GetTypeId ()
{
...
double
PerformanceSimpleModel::GetExecTime () const
{
  NS_LOG_FUNCTION (this);
  return m_execTime;
}
...
double
PerformanceSimpleModel::GetThroughput () const
{
  NS_LOG_FUNCTION (this);
  return m_throughput;
}
...
double
PerformanceSimpleModel::GetA () const
{
  NS_LOG_FUNCTION (this);
  return m_A;
}
...
double
PerformanceSimpleModel::GetB () const
{
  NS_LOG_FUNCTION (this);
  return m_B;
}
...
double
PerformanceSimpleModel::GetC () const
{
  NS_LOG_FUNCTION (this);
  return m_C;
}
...
double
PerformanceSimpleModel::GetDataSize () const
{
  NS_LOG_FUNCTION (this);
  return m_dataSize;
}
...
double
PerformanceSimpleModel::GetPacketSize () const
{
  NS_LOG_FUNCTION (this);
  return m_packetSize;
}
...
PerformanceSimpleModel::SetApplication (std::string m_appName, const DoubleValue &v0)
{
  ...
  if (m_deviceType == "RaspberryPi")
  {
    if (m_appName == "AdaBoost")
    {
      ...
      m_B = 5.32 * pow(10, -2);
      m_C = 2.40 * pow(10, 1);
      m_execTime = std::max(0.1, (m_A * pow(m_dataSize, 2) / 1'000'000 + m_B * m_dataSize / 1'000) + m_C);
    }
    ...
    else if(m_appName == "LinearRegression")
    {
      ...
      m_B = 6.154 * pow(10, -6);
      m_C = 0.12 * pow(10, 0);
      m_execTime = std::max(0.1, (m_A * pow(m_dataSize, 2) / 1'000'000 + m_B * m_dataSize) + m_C);
    }
    else
    {
  ...
  else if(m_deviceType == "Server")
  {
    ...
    else if (m_appName == "AffinityPropagation")
    {
      ...
      m_B = 2.69 * pow(10, -1);
      m_C = -1.40 * pow(10, 0);
      m_execTime = std::max(0.1, (m_A* pow(m_dataSize, 2) / 1'000'000 + m_B * m_dataSize / 1000) + m_C);
    }
    else if (m_appName == "Birch")
    {
      m_A = 1.13 * pow(10, -3);
  ...
  else if (m_deviceType == "Arduino")
  {
    if (m_appName == "MedianFilter")
    {
      ...
      m_B = 1.0 * pow(10, -1);
      m_C = 0.5 * pow(10, 0);
      m_execTime = std::max(0.1, (m_B * m_dataSize) / 1'000 + m_C);
    }
    else
    {
      NS_FATAL_ERROR ("AppPowerModel:Undefined application for this device: " << m_appName);
...
void
PerformanceSimpleModel::DoDispose ()
{
  NS_LOG_FUNCTION (this);
}
